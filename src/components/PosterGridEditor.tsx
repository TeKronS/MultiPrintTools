...
export default function PosterGridEditor() {
  ...
  const handleWidthChange = (val: string, isDraft: boolean) => {
    ...
    if (num > 0) {
      const opt = calculateOptimizedGrid(num, newHeight, isDraft ? draftPaperSize : paperSize, isDraft ? draftOverlap : overlap, isDraft ? draftMarginV : marginV, isDraft ? draftMarginH : marginH);
      setR(opt.rows); setC(opt.cols); setO(opt.orientation);
    }
  };
  ...
  const handleHeightChange = (val: string, isDraft: boolean) => {
    ...
    if (num > 0) {
      const opt = calculateOptimizedGrid(newWidth, num, isDraft ? draftPaperSize : paperSize, isDraft ? draftOverlap : overlap, isDraft ? draftMarginV : marginV, isDraft ? draftMarginH : marginH);
      setW(newWidth.toFixed(1)); setR(opt.rows); setC(opt.cols); setO(opt.orientation);
    }
  };
  ...
  const handleImageUpload = (file: File, url: string) => {
    ...
    img.onload = () => {
      ...
      const calcSize = (orient: 'portrait' | 'landscape') => {
        const p = orient === 'portrait' ? { w: Math.min(paperBase.width, paperBase.height), h: Math.max(paperBase.width, paperBase.height) } : { w: Math.max(paperBase.width, paperBase.height), h: Math.min(paperBase.width, paperBase.height) };
        ...
        let finalW, finalH;
        if (targetGridW / targetGridH > aspect) { finalH = targetGridH; finalW = finalH * aspect; } else { finalW = targetGridW; finalH = finalW / aspect; }
      ...
      setRows(2); setCols(2); setOrientation(best.orient); setTargetWidth((best.w / 10).toFixed(1)); setTargetHeight((best.h / 10).toFixed(1));
    };
  };

  const handleMenuOpenChange = (open: boolean) => {
    if (open) {
      setDraftRows(rows); setDraftCols(cols); setDraftOverlap(overlap); setDraftMarginV(marginV); setDraftMarginH(marginH); setDraftPaperSize(paperSize); setDraftOrientation(orientation); setDraftShowGuides(showGuides); setDraftShowOutline(showOutline); setDraftTargetWidth(targetWidth); setDraftTargetHeight(targetHeight);
    } else {
      setRows(draftRows); setCols(draftCols); setOverlap(draftOverlap); setMarginV(draftMarginV); setMarginH(draftMarginH); setPaperSize(draftPaperSize); setOrientation(draftOrientation); setShowGuides(draftShowGuides); setShowOutline(draftShowOutline); setTargetWidth(draftTargetWidth); setTargetHeight(draftTargetHeight);
    }
    setIsMenuOpen(open);
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const paper = activeOrientation === 'portrait' ? { width: Math.min(paperBase.width, paperBase.height), height: Math.max(paperBase.width, paperBase.height) } : { width: Math.max(paperBase.width, paperBase.height), height: Math.min(paperBase.width, paperBase.height) };
      const { appliedMH, appliedMV } = getAppliedMargins(activeOrientation, activeMarginV, activeMarginH);
      const pdf = new jsPDF({ orientation: activeOrientation === 'portrait' ? 'p' : 'l', unit: 'mm', format: paperBase.format as any });
      const printableW = paper.width - (appliedMH * 20);
      const printableH = paper.height - (appliedMV * 20);
      ...
      for (let r = 0; r < activeRows; r++) {
        for (let c = 0; c < activeCols; c++) {
          ...
          if (sw > 0 && sh > 0) {
            ...
            pdf.addImage(canvas.toDataURL('image/jpeg', 0.95), 'JPEG', panelX, panelY, visibleW_mm, visibleH_mm);
            if (activeShowOutline) { pdf.setDrawColor(200); pdf.setLineWidth(0.1); pdf.rect(panelX, panelY, visibleW_mm, visibleH_mm); }
          }
    ...
    } catch (e) { toast({ variant: "destructive", title: "Error", description: "No se pudo generar el PDF." }); } finally { setIsExporting(false); }
  };
  ...
  const getMeasures = (r: number, c: number, ov: number, mV: number, mH: number, pS: string, orient: 'portrait' | 'landscape', tW: string, tH: string) => {
    ...
    const paperBase = PAPER_DIMENSIONS[pS];
    const paper = orient === 'portrait' ? { width: Math.min(paperBase.width, paperBase.height), height: Math.max(paperBase.width, paperBase.height) } : { width: Math.max(paperBase.width, paperBase.height), height: Math.min(paperBase.width, paperBase.height) };
    const { appliedMH, appliedMV } = getAppliedMargins(orient, mV, mH);
    const printableW = paper.width - (appliedMH * 20);
    const printableH = paper.height - (appliedMV * 20);
    const overlapMm = ov * 10;
    ...
    return { imgW: (finalW_mm / 10).toFixed(1), imgH: (finalH_mm / 10).toFixed(1), printableW: (printableW / 10).toFixed(1), printableH: (printableH / 10).toFixed(1), blankW: Math.max(0, (totalGridW - finalW_mm) / 10).toFixed(1), blankH: Math.max(0, (totalGridH - finalH_mm) / 10).toFixed(1) };
  };
  ...
  const renderSettings = (currentRows: number, currentCols: number, currentOverlap: number, currentMarginV: number, currentMarginH: number, currentPaperSize: string, currentOrientation: 'portrait' | 'landscape', currentShowGuides: boolean, currentShowOutline: boolean, curWidth: string, curHeight: string, info: any, isMobile?: boolean) => (
    <div className={cn("space-y-4", isMobile ? "p-4 pt-1" : "p-4")}>
      ...
      <div className="space-y-4">
        <div className="bg-card p-4 rounded-xl border border-primary/10 shadow-sm space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input type="number" value={curWidth} onChange={(e) => handleWidthChange(e.target.value, !!isMobile)} min="0.1" step="0.1" className="h-9 font-black text-sm text-primary bg-primary/5 border-primary/20" />
            </div>
            <div className="space-y-1">
              <Label className="text-[8px] font-bold text-muted-foreground uppercase">{t.height}</Label>
              <Input type="number" value={curHeight} onChange={(e) => handleHeightChange(e.target.value, !!isMobile)} min="0.1" step="0.1" className="h-9 font-black text-sm text-primary bg-primary/5 border-primary/20" />
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => handleRowsGridChange(Math.max(1, currentRows - 1), !!isMobile)}><Minus className="h-3 w-3" /></Button>
            <Slider value={[currentRows]} onValueChange={(v) => handleRowsGridChange(v[0], !!isMobile)} min={1} max={MAX_GRID_DIM} step={1} className="flex-1" />
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => handleRowsGridChange(Math.min(MAX_GRID_DIM, currentRows + 1), !!isMobile)}><Plus className="h-3 w-3" /></Button>
          </div>
        </div>
        <div className="space-y-2">
          <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => handleColsGridChange(Math.max(1, currentCols - 1), !!isMobile)}><Minus className="h-3 w-3" /></Button>
            <Slider value={[currentCols]} onValueChange={(v) => handleColsGridChange(v[0], !!isMobile)} min={1} max={MAX_GRID_DIM} step={1} className="flex-1" />
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => handleColsGridChange(Math.min(MAX_GRID_DIM, currentCols + 1), !!isMobile)}><Plus className="h-3 w-3" /></Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            ...
            <Select value={currentPaperSize} onValueChange={(v) => { if (isMobile) setDraftPaperSize(v); else setPaperSize(v); setTimeout(() => syncGridFromTechnicalSettings(!!isMobile), 0); }}>
              ...
              <SelectContent>{Object.keys(PAPER_DIMENSIONS).map(key => <SelectItem key={key} value={key} className="text-xs font-bold">{key}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Select value={currentOrientation} onValueChange={(v: any) => { if (isMobile) setDraftOrientation(v); else setOrientation(v); setTimeout(() => syncGridFromTechnicalSettings(!!isMobile), 0); }}>
              <SelectTriggerUI className="h-8 rounded-lg text-xs font-bold bg-card shadow-sm border border-border/10"><SelectValue /></SelectTriggerUI>
              <SelectContent>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <Label className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2 bg-card px-2 py-0.5 rounded-md shadow-sm border border-border/10"><Scissors className="h-3 w-3" /> {t.overlap}</Label>
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-amber-500/10 hover:text-amber-600" onClick={() => { const newVal = Math.max(0, parseFloat((currentOverlap - 0.1).toFixed(1))); if (isMobile) setDraftOverlap(newVal); else setOverlap(newVal); syncGridFromTechnicalSettings(!!isMobile); }}><Minus className="h-3 w-3" /></Button>
            <Slider value={[currentOverlap]} onValueChange={(v) => { if (isMobile) setDraftOverlap(v[0]); else setOverlap(v[0]); syncGridFromTechnicalSettings(!!isMobile); }} min={0} max={10} step={0.1} className="flex-1" />
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-amber-500/10 hover:text-amber-600" onClick={() => { const newVal = Math.min(10, parseFloat((currentOverlap + 0.1).toFixed(1))); if (isMobile) setDraftOverlap(newVal); else setOverlap(newVal); syncGridFromTechnicalSettings(!!isMobile); }}><Plus className="h-3 w-3" /></Button>
          </div>
        </div>
        <div className="space-y-2">
          <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => { const newVal = Math.max(0, parseFloat((currentMarginV - 0.1).toFixed(1))); if (isMobile) setDraftMarginV(newVal); else setMarginV(newVal); syncGridFromTechnicalSettings(!!isMobile); }}><Minus className="h-3 w-3" /></Button>
            <Slider value={[currentMarginV]} onValueChange={(v) => { if (isMobile) setDraftMarginV(v[0]); else setMarginV(v[0]); syncGridFromTechnicalSettings(!!isMobile); }} min={0} max={5} step={0.1} className="flex-1" />
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => { const newVal = Math.min(5, parseFloat((currentMarginV + 0.1).toFixed(1))); if (isMobile) setDraftMarginV(newVal); else setMarginV(newVal); syncGridFromTechnicalSettings(!!isMobile); }}><Plus className="h-3 w-3" /></Button>
          </div>
        </div>
        <div className="space-y-2">
          <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => { const newVal = Math.max(0, parseFloat((currentMarginH - 0.1).toFixed(1))); if (isMobile) setDraftMarginH(newVal); else setMarginH(newVal); syncGridFromTechnicalSettings(!!isMobile); }}><Minus className="h-3 w-3" /></Button>
            <Slider value={[currentMarginH]} onValueChange={(v) => { if (isMobile) setDraftMarginH(v[0]); else setMarginH(v[0]); syncGridFromTechnicalSettings(!!isMobile); }} min={0} max={5} step={0.1} className="flex-1" />
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0 hover:bg-primary/10 hover:text-primary" onClick={() => { const newVal = Math.min(5, parseFloat((currentMarginH + 0.1).toFixed(1))); if (isMobile) setDraftMarginH(newVal); else setMarginH(newVal); syncGridFromTechnicalSettings(!!isMobile); }}><Plus className="h-3 w-3" /></Button>
          </div>
        </div>
        <div className="space-y-3 pt-2">
          <div className="flex items-center justify-between">
            <Label className="text-[10px] font-black uppercase text-muted-foreground cursor-pointer flex items-center gap-2 bg-card px-2 py-0.5 rounded-md shadow-sm border border-border/10" htmlFor={isMobile ? "draft-guides" : "guides"}><Layers className="h-3 w-3" /> {t.guides}</Label>
          ...
          <div className="flex items-center justify-between">
            <Label className="text-[10px] font-black uppercase text-muted-foreground cursor-pointer flex items-center gap-2 bg-card px-2 py-0.5 rounded-md shadow-sm border border-border/10" htmlFor={isMobile ? "draft-outline" : "outline"}><Square className="h-3 w-3" /> {t.outline}</Label>
      ...
      {info && (
        <div className="p-3 bg-card/90 rounded-xl border border-border shadow-xl space-y-2">
          <div className="flex items-center gap-2 mb-0.5"><Ruler className="h-3 w-3 text-muted-foreground" /><span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{lang === 'es' ? 'Resumen Técnico' : 'Technical Summary'}</span></div>
          <div className="grid grid-cols-2 gap-3">
            <div><span className="text-[8px] font-bold text-muted-foreground uppercase block">{lang === 'es' ? 'Imagen' : 'Image'}</span><span className="text-xs font-black text-primary">{info.imgW} x {info.imgH} cm</span></div>
            <div><span className="text-[8px] font-bold text-muted-foreground uppercase block">{t.panelArea}</span><span className="text-xs font-black text-foreground">{info.printableW} x {info.printableH} cm</span></div>
          </div>
          <Separator className="opacity-20" />
          <div className="flex justify-between items-center"><span className="text-[8px] font-bold text-muted-foreground uppercase">{t.blankSpace}</span><span className={cn("text-[10px] font-black", Number(info.blankW) > 0 || Number(info.blankH) > 0 ? "text-amber-600" : "text-muted-foreground")}>+{info.blankW}w / +{info.blankH}h cm</span></div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-screen w-full font-body bg-background text-foreground overflow-hidden">
      <header className="h-16 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between px-4 lg:px-6 z-50 shadow-sm shrink-0 sticky top-0">
        <div className="flex items-center gap-2 lg:gap-6">
          <Link href="/"><Button variant="ghost" size="sm" className="gap-2 font-bold text-muted-foreground px-2"><ChevronLeft className="h-4 w-4" /><span className="hidden lg:inline">Inicio</span></Button></Link>
          <div className="flex items-center gap-2 lg:gap-3"><div className="w-6 h-6 lg:w-8 lg:h-8 relative overflow-hidden rounded-lg shadow-sm bg-white border border-border/10"><Image src={logo} alt="Logo" fill className="object-contain" /></div><h1 className="text-lg lg:text-xl font-headline font-black tracking-tighter text-primary uppercase">CUADRÍCULA POSTER<span className="text-amber-600">.</span></h1></div>
          <Separator orientation="vertical" className="h-8 hidden lg:block" />
        </div>
        <div className="flex items-center gap-2 lg:gap-4"><ThemeToggle /><LanguageSelector language={lang} setLanguage={setLang} /><Button className="hidden lg:flex bg-primary hover:bg-primary/90 text-white font-black gap-2 h-10 px-6 rounded-xl shadow-md transition-all active:scale-95 text-xs" onClick={handleExport} disabled={!image || isExporting}>{isExporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileDown className="h-4 w-4" />}{isExporting ? "..." : t.export}</Button></div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        <section className={cn("flex-1 relative bg-muted/30 flex flex-col items-center overflow-y-auto", !image ? "justify-start py-8" : "justify-start")}>
          {!image ? (
            ...
            <>
              {physicalInfo && (
                <div className="absolute top-4 lg:top-6 left-1/2 -translate-x-1/2 z-20 pointer-events-none w-full max-w-5xl px-4 lg:px-8"><div className="bg-background/80 backdrop-blur-xl border border-primary/20 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.1)] px-4 lg:px-6 py-2 lg:py-3 rounded-2xl flex items-center justify-between pointer-events-auto animate-fade-in"><div className="flex items-center gap-3 lg:gap-6"><div className="flex flex-col"><span className="text-[6px] lg:text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">{t.finalMeasures}</span><div className="flex items-center gap-1.5 lg:gap-2"><Maximize2 className="h-2.5 w-2.5 lg:h-3.5 lg:w-3.5 text-primary" /><span className="text-xs lg:text-base font-black text-foreground">{physicalInfo.imgW} x {physicalInfo.imgH} cm</span></div></div><Separator orientation="vertical" className="h-6 lg:h-8 opacity-50" /><div className="flex flex-col"><span className="text-[6px] lg:text-[9px] font-black text-muted-foreground uppercase tracking-widest monolithic mb-1">Paneles</span><div className="flex items-center gap-1.5 lg:gap-2"><Layers className="h-2.5 w-2.5 lg:h-3.5 lg:w-3.5 text-amber-600" /><span className="text-xs lg:text-base font-black text-foreground">{rows * cols} <span className="hidden sm:inline">{lang === 'es' ? 'HOJAS' : 'SHEETS'}</span></span></div></div></div><div className="flex items-center gap-2 lg:gap-4"><div className="hidden sm:flex flex-col items-end mr-2"><span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">{t.paperSize}</span><span className="text-[11px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-md">{paperSize} {orientation === 'portrait' ? '(V)' : '(H)'}</span></div><Button variant="secondary" size="sm" onClick={() => setImage(null)} className="h-8 lg:h-10 text-[10px] lg:text-xs font-black uppercase gap-2 rounded-xl border border-border/50 shadow-sm"><RefreshCcw className="h-3.5 w-3.5" /><span className="hidden xs:inline">{lang === 'es' ? 'Cambiar Imagen' : 'Change Image'}</span></Button></div></div></div>
              )}
              <div className="w-full h-full p-4 lg:p-8 flex flex-col pt-16 lg:pt-24 overflow-y-auto"><div className="flex-1 min-h-0"><MuralCanvas imageUrl={image.url} rows={deferredRows} cols={deferredCols} overlap={deferredOverlap} marginV={deferredMarginV} marginH={deferredMarginH} paperSize={paperSize} orientation={orientation} showGuides={showGuides} showOutline={showOutline} imageWidth={parseFloat(targetWidth)} imageHeight={parseFloat(targetHeight)} /></div><div className="lg:hidden mt-4 pb-4 w-full max-w-xl mx-auto shrink-0"><Button className="w-full h-12 bg-primary hover:bg-primary/90 text-white font-black gap-3 rounded-2xl shadow-xl transition-all active:scale-95 text-sm uppercase tracking-widest" onClick={handleExport} disabled={!image || isExporting}>{isExporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileDown className="h-4 w-4" />}{isExporting ? "Generando PDF..." : t.export}</Button></div></div>
            </>
          )}
        </section>
        <aside className="hidden lg:block w-80 border-l border-border bg-background overflow-y-auto shadow-xl z-10 shrink-0 h-full">
          {renderSettings(rows, cols, overlap, marginV, marginH, paperSize, orientation, showGuides, showOutline, targetWidth, targetHeight, physicalInfo)}
        </aside>
      </main>
      {image && (
        <div className="lg:hidden fixed bottom-6 right-6 z-[100] pointer-events-auto"><Sheet open={isMenuOpen} onOpenChange={handleMenuOpenChange}><Button size="icon" className="h-14 w-14 rounded-full shadow-2xl bg-primary text-white hover:bg-primary/90 transition-all active:scale-95 border-4 border-white" onPointerDown={(e) => { e.preventDefault(); e.stopPropagation(); handleMenuOpenChange(!isMenuOpen); }}><Settings2 className="h-6 w-6" /></Button><SheetContent side="right" className="w-[85%] sm:w-[400px] p-0 bg-background/80 backdrop-blur-xl shadow-2xl overflow-hidden flex flex-col"><SheetHeader className="sr-only"><SheetTitle>{t.gridSettings}</SheetTitle><SheetDescription>Panel de ajustes para la cuadrícula del poster</SheetDescription></SheetHeader><div className="p-3 bg-primary/5 flex flex-col gap-3"><Button className="w-full h-12 bg-primary hover:bg-primary/90 text-white font-black gap-3 rounded-xl shadow-lg transition-all active:scale-95 text-xs uppercase tracking-widest" onClick={handleExport} disabled={!image || isExporting}>{isExporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileDown className="h-4 w-4" />}{isExporting ? "..." : t.export}</Button></div><div className="flex-1 overflow-y-auto">{renderSettings(draftRows, draftCols, draftOverlap, draftMarginV, draftMarginH, draftPaperSize, draftOrientation, draftShowGuides, draftShowOutline, draftTargetWidth, draftTargetHeight, draftInfo, true)}</div></SheetContent></Sheet></div>
      )}
    </div>
  );
}
